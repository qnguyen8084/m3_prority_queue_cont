...
public class MyPriorityQueue<T extends Comparable<? super T>>{
    ...
    public void swap(int firstIndex, int secondIndex) {
        T temp = queueObject.get(firstIndex);
        queueObject.set(firstIndex, queueObject.get(secondIndex));
        queueObject.set(secondIndex, temp);
    }
    ...
    public void sink() {
        ...
        while (index < arraySize) {
            ...
            if (rightChildIndex < arraySize
                    && queueObject.get(rightChildIndex).compareTo(queueObject.get(leftChildIndex)) > 0) {
